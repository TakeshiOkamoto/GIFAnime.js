...
TGIFAnime.prototype = {    
    ...
    // Graphic Control Extension用のビット処理
    _Octet_GraphicControl : function (r,dm,uif,tcf) {
        var result = 0;        
         
        // [3Bit]未使用(Reserved)
        result |=  r << 5;
        
        // [3Bit]処理方法(Disposal Method)
        // 0:指定なし 1:現状維持 2:背景色で塗りつぶし 3: 前の状態に戻す 
        result |=  dm << 2;        
        
        // [1Bit]ユーザー入力(User Input Flag)
        // 0:受けない 1: 受ける
        result |=  uif << 1;    
        
        // [1Bit]透過処理(Transparent Color Flag)
        // 0:透過しない 1:透過する
        result |=  tcf;    
    ...
    // イメージデータの書き込み
    _wrtieImagedata: function (F,firstflg,r,g,b) {      
        ...
        if(!this.transflg){  
          // 透過なし
          F.WriteByte(this._Octet_GraphicControl(0,this.overlaid,0,0)); 
        }else{
          // 透過あり
          F.WriteByte(this._Octet_GraphicControl(0,this.overlaid,0,1)); 
        }
        
        // アニメの待ち時間(100 = 1秒)
        F.WriteWord(this.delaytime); 
        ...
        // 処理方法(Disposal Method)
        var lctf,slct;
        var len = this.palette.length;
        if (len >= 1 && len <=2){          lctf = slct = 0;}
        else if (len >= 3   && len <=4)  { lctf = slct = 1;}
        else if (len >= 5   && len <=8)  { lctf = slct = 2;}
        else if (len >= 9   && len <=16) { lctf = slct = 3;}
        else if (len >= 17  && len <=32) { lctf = slct = 4;}
        else if (len >= 33  && len <=64) { lctf = slct = 5;}
        else if (len >= 65  && len <=128){ lctf = slct = 6;}
        else if (len >= 129 && len <=256){ lctf = slct = 7;}                
        ...
        if (firstflg){
          ...
          // 固有パレットやインタレースなど
          F.WriteByte(this._Octet_ImageData(1,0,0,0,slct));  
    ...
    SaveToStream: function (r,g,b) {
        ...
        // 1枚目のイメージ        
        this.raw    = this.imagelist[0].data;
        this.width  = this.imagelist[0].width;
        this.height = this.imagelist[0].height;  
        this.colors = this.imagelist[0].colors;       // 色情報配列
        this.transflg  = this.imagelist[0].transflg;  // 透明フラグ  
        this.overlaid  = this.imagelist[0].overlaid;  // 重ね方         
        this.delaytime = this.imagelist[0].delaytime; // 表示時間 
        ...
        if(this.repeatcount != 1){
          ...
          // NETSCAPE2.0
          F.WriteByte(0x4E);F.WriteByte(0x45);F.WriteByte(0x54);F.WriteByte(0x53);        
          F.WriteByte(0x43);F.WriteByte(0x41);F.WriteByte(0x50);F.WriteByte(0x45);        
          F.WriteByte(0x32);F.WriteByte(0x2E);F.WriteByte(0x30);
          ...
          F.WriteByte(0x03);
            ...
            if(this.repeatcount == 0){
              ...
              // 繰り返し回数は「繰り返し回数 - 1」を入力する
              F.WriteWord(this.repeatcount-1);              
            }
        ...
        // 1枚目 
        this._wrtieImagedata(F,true,r,g,b);
        
        // 2枚目以降
        for (var i=1;i<this.imagelist.length;i++){
          this.raw      = this.imagelist[i].data;
          this.width    = this.imagelist[i].width;
          this.height   = this.imagelist[i].height; 
          this.colors   = this.imagelist[i].colors;  
          this.transflg  = this.imagelist[i].transflg;
          this.overlaid  = this.imagelist[i].overlaid;
          this.delaytime = this.imagelist[i].delaytime;
                       
          this._getColorPalette();
          
          this._wrtieImagedata(F,false,r,g,b);
        }                          
